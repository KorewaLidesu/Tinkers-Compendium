        ...
        for (BlockPos.MutableBlockPos blockpos$mutableblockpos1 : BlockPos.getAllInBoxMutable(pos.add(-f, -1.0D, -f), pos.add(f, -1.0D, f))) {
            if (blockpos$mutableblockpos1.distanceSqToCenter(living.posX, living.posY, living.posZ) <= (double) (f * f)) {
                ...
                if (iblockstate.getMaterial() == Material.AIR) {
                    ...
                    if ((iblockstate1.getBlock() == fromBlock || iblockstate1.getBlock() == fromBlockFlowing) && iblockstate1.getValue(BlockLiquid.LEVEL).intValue() == 0 && worldIn.mayPlace(toBlock, blockpos$mutableblockpos1, false, EnumFacing.DOWN, null)) {
