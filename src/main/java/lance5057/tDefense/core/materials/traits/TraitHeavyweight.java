        ...
        if (e.getSource() == DamageSource.FALL) {
            ...
            for (int x = (int) (-1 * ent.fallDistance); x < 1 * ent.fallDistance; x++) {
                for (int y = (int) (-1 * ent.fallDistance); y < 1 * ent.fallDistance; y++) {
                    ...
                    ent.world.spawnParticle(EnumParticleTypes.BLOCK_CRACK, underPlayer.getX(), underPlayer.getY(), underPlayer.getZ(), ent.world.rand.nextInt(5), Math.abs((x + y) + ent.fallDistance), ent.world.rand.nextInt(5));
                }
            }
